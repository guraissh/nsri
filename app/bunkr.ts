 ...
 */
export async function bnk_getCacheStats(): Promise<CacheStats | null> {
 ...
 */
export async function bnk_clearAllCache(): Promise<ClearCacheResult> {
  const baseUrl = getBaseUrl();
 ...
 */
export async function bnk_getAlbum(albumUrl: string): Promise<BunkrAlbumResponse> {
  const baseUrl = getBaseUrl();

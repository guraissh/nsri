...
import { bnk_getAlbum } from "./bunkr";
import { rg_getUserGifs, rg_searchGifs, type RedgifsGifsResponse } from "./redgifs";
...
export const getBunkrMedia = async (
  ...
  try {
    ...
    const data = await bnk_getAlbum(albumUrl);
...
export const getRedgifsMedia = async (
  ...
  try {
    ...
    if (username) {
      ...
      data = await rg_getUserGifs(username, normalizedOrder, page, count);
    } else if (tags) {
      // Search by tags only
      console.log(`Searching RedGifs by tags: ${tags}`);
      data = await rg_searchGifs(tags, normalizedOrder, page, count);
    } else {

import { createCookie, createCookieSessionStorage, type Cookie } from "react-router";
...
try {
  ...
  browserCookies = parsedCookies.map(c => createCookie(c.name, { 
    ...c, 
...
const { getSession, commitSession, destroySession } = createCookieSessionStorage<SessionData, SessionFlashData>({
  cookie: sessionCookie ? createCookie(sessionCookie.name, {
...
export { getSession, commitSession, destroySession, browserCookies, parsedCookies };
